...
class BaseTrainer(abc.ABC):
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            fwd_out = self.model(
                ...
                near_far=data['near_fars'], timestamps=data['timestamps'], camids=data['camids'], global_step=self.global_step)
    ...
    def _move_data_to_device(self, data):
        ...
        if "camids" in data:
            data["camids"] = data["camids"].to(self.device)
    ...
    def evaluate_metrics(self,
        ...
        if save_outputs:
            ...
            pred_name = out_name + "-pred"
            write_png(os.path.join(self.log_dir, pred_name + ".png"), out_img_only_np)           
            if out_depth is not None:
    ...
    def load_model(self, checkpoint_data, training_needed: bool = True):
        ...
        if self.test_optim:
            ...
            log.info("=> Loaded model state from checkpoint without cam offset")

            self.global_step = 0
            log.info(f"=> Started step {self.global_step}")
