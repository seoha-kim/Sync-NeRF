...
def OctreeRender_trilinear_fast(rays, tensorf, std_train, chunk=4096, N_samples=-1, ndc_ray=False, white_bg=True, is_train=False, device='cuda', rgb_train=None, cam_id=None, iteration=None,
                                use_time='all', time=None, temporal_indices=None, static_branch_only=False, with_grad=True, simplify=False, remove_foreground=False, test_cam_offset=0., **kwargs):
    # tiktok = TicTok()
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        current_values = tensorf(rays_chunk, is_train=is_train, white_bg=white_bg, ndc_ray=ndc_ray, cam_id=cam_id, iteration=iteration, 
                                 N_samples=N_samples, rgb_train=rgb_train, temporal_indices=temporal_indices,
                                 static_branch_only=static_branch_only, std_train=std_train,remove_foreground=remove_foreground, test_cam_offset=test_cam_offset, **kwargs)
...
@torch.no_grad()
def evaluation(test_dataset, tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,iteration=None,
               white_bg=False, ndc_ray=False, compute_extra_metrics=True, device='cuda', simplify=False,
               static_branch_only=False, remove_foreground=False, cam_id=None):
    PSNRs, PSNRs_pf, PSNRs_STA, PSNRs_DY, rgb_maps, depth_maps = [], [], [], [], [], []
    ssims,l_alex,l_vgg=[],[],[]
    ssims_dy,l_alex_dy,l_vgg_dy=[],[],[]
    os.makedirs(savePath, exist_ok=True)
    os.makedirs(savePath+"/rgbd", exist_ok=True)

    if savePath.split('/')[-2] == 'imgs_test_all' and args.ckpt:
        os.makedirs(savePath+"images/rgb", exist_ok=True)
        os.makedirs(savePath+"images/depth", exist_ok=True)
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        retva = renderer(rays, tensorf,  std_train=None, chunk=args.batch_size//2, N_samples=N_samples, ndc_ray=ndc_ray, white_bg = white_bg, device=device, with_grad=False,
                         simplify=simplify, static_branch_only=static_branch_only, remove_foreground=remove_foreground, iteration=iteration, cam_id=cam_id)
        ...
        if len(test_dataset.all_rgbs):
            ...
            gt_dynamic_rgb = gt_rgb.clone()
            gt_dynamic_rgb[~test_dataset.dynamic_mask[0], ...] = 0
            dynamic_rgb_map = retva.rgb_map.clone()
            dynamic_rgb_map[~test_dataset.dynamic_mask[0], ...] = 0
            dynamic_depth_map = np.array(retva.depth_map).copy()
            dynamic_depth_map[:,~test_dataset.dynamic_mask[0], :] = 0
            ...
            if not static_branch_only:
                ...
                loss_dynamic = ((dynamic_rgb_map[test_dataset.dynamic_mask[0], ...] - gt_dynamic_rgb[test_dataset.dynamic_mask[0], ...]) ** 2).mean(dim=0).mean(dim=1).mean()
            loss_static = torch.mean((retva.static_rgb_map - gt_static_rgb) ** 2)
            if not static_branch_only:
                PSNRs_DY.append(-10.0 * np.log(loss_dynamic.item()) / np.log(10.0))
        ...
        if not static_branch_only:
            for rgb_map, depth_map, name, is_video in [(retva.static_rgb_map, retva.static_depth_map, 'static', False),
                                                       (dynamic_rgb_map, dynamic_depth_map, 'moving', True),
                ...
                if is_video:
                    ...
                    if savePath.split('/')[-2] == 'imgs_test_all' and name == 'comp' and args.ckpt:
                        imgs = [Image.fromarray(img) for img in np.stack(rgb_maps)]
                        for i, img in enumerate(imgs):
                            img.save(f"{savePath}images/rgb/{str(i):.4s}.jpeg")

                        depths = [Image.fromarray(img) for img in np.stack(depth_maps)]
                        for i, depth in enumerate(depths):
                            depth.save(f"{savePath}images/depth/{str(i):.4s}.jpeg")
                else:
    ...
    if not static_branch_only:
        ...
        psnr_dy = np.mean(np.asarray(PSNRs_DY))
    ...
    if not static_branch_only:
        ...
        np.savetxt(f'{savePath}/{prtx}mean.txt', np.asarray([psnr, psnr_pf, psnr_dy, psnr_sta, ssim, l_a, l_v, flip_output, jod_output])), #flip_output, jod_output]))
    ...
    if not static_branch_only:
        print('PSNR:{:.6f}, PSNR_PERFRAME:{:.6f}, PSNR_DY:{:.6f}, PSNR_STA:{:.6f}'.format(psnr, psnr_pf, psnr_dy, psnr_sta))
...
@torch.no_grad()
def evaluation_path(test_dataset, tensorf, args, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1, iteration=None,
                    white_bg=False, ndc_ray=False, device='cuda', static_branch_only=False, temporal_sampler=None,
                    remove_foreground=False, start_idx=0, nodepth=True, cam_id=torch.Tensor([0.])):
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        retva = renderer(rays, tensorf, std_train=None, chunk=args.batch_size*4, N_samples=N_samples, cam_id=cam_id,
                         ndc_ray=ndc_ray, white_bg = white_bg, device=device, with_grad=False, iteration=iteration,
