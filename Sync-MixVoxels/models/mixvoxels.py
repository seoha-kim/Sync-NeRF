...
class MixVoxels(torch.nn.Module):
    def __init__(self, args, aabb, gridSize, device, density_n_comp = 8, appearance_n_comp = 24, app_dim = 27, #27
                    ...
                    dynamic_pool_kernel_size=1, time_head='dyrender', camlist=None, 
        ...
        self.TimeHead = {'dyrender': DyRender, 'directdyrender': DirectDyRender, 'timemlprender':TimeMLPRender, 'forrier': ForrierDyRender}[time_head]
        ...
        self.init_cam_offset(args, self.offset_size, device)
    ...
    def init_cam_offset(self, args, offset_size, device):
        ...
        print(f'cam offset is initialized as {self.cam_offset}')
        return self.cam_offset
        

    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        if self.time_head == 'dyrender': # concat
            self.renderModule = self.TimeHead(self.app_dim, viewpe=view_pe, using_view=True, n_time_embedding=self.n_time_embedding, featureD=featureC,
                                            total_time=self.n_frames).to(device)
        if self.time_head == 'timemlprender': # time mlp
            self.renderModule = self.TimeHead(self.app_dim, args=self.args, viewpe=view_pe, using_view=True, n_time_embedding=self.n_time_embedding, featureD=featureC, camoffset=self.cam_offset,
                                            total_time=self.n_frames, net_spec=self.args.netspec_dy_color).to(device)
        elif self.time_head == 'directdyrender': # inner-product
            self.renderModule = self.TimeHead(self.app_dim, viewpe=view_pe, using_view=True, n_time_embedding=self.n_time_embedding, featureD=featureC, args=self.args,
                                            total_time=self.n_frames, net_spec=self.args.netspec_dy_color).to(device)
    ...
    def init_render_den_func(self, densityMode, pos_pe, featureD, device):
        if self.time_head == 'dyrender':  # concat
            self.renderDenModule = self.TimeHead(self.den_dim, viewpe=0, using_view=False, n_time_embedding=self.n_time_embedding, featureD=featureD,
                                            total_time=self.n_frames).to(device)
        if self.time_head == 'timemlprender': # time mlp
            self.renderDenModule = self.TimeHead(self.den_dim, args=self.args, viewpe=0, using_view=False, n_time_embedding=self.n_time_embedding, featureD=featureD, camoffset=self.cam_offset,
                                            total_time=self.n_frames, net_spec=self.args.netspec_dy_density).to(device)
        elif self.time_head == 'directdyrender':    # inner-product
            self.renderDenModule = self.TimeHead(self.den_dim, viewpe=0, using_view=False, n_time_embedding=self.n_time_embedding, featureD=featureD, args=self.args,
                                            total_time=self.n_frames, net_spec=self.args.netspec_dy_density).to(device)
    ...
    def save(self, path, iters, cam_offset, optimizer, static_optimizer, dy_optimizer):
        kwargs = self.get_kwargs()
        ckpt = {'kwargs': kwargs, 'state_dict': self.state_dict(), 'iters': iters, 'cam_offset': cam_offset, 
        'optimizer':optimizer, 'static_optimizer': static_optimizer, 'dy_optimizer':dy_optimizer}
    ...
    def load(self, ckpt):
        ...
        print('########### load state dict with mlp keys ########### ')


    def load_without_mlp(self, ckpt):
        if 'alphaMask.aabb' in ckpt.keys():
            length = np.prod(ckpt['alphaMask.shape'])
            alpha_volume = torch.from_numpy(np.unpackbits(ckpt['alphaMask.mask'])[:length].reshape(ckpt['alphaMask.shape']))
            self.alphaMask = AlphaGridMask(self.device, ckpt['alphaMask.aabb'].to(self.device), alpha_volume.float().to(self.device))
        ...
        for key in list(ckpt['state_dict'].keys()):
            ...
            if key.startswith('renderDenModule'):
                ckpt['state_dict'].pop(key)
        self.load_state_dict(ckpt['state_dict'], strict=False)
        print('########### load state dict without mlp keys ########### ')
    ...
    def compute_temporal_alpha(self, xyz_locs, length=1):
        ...
        if ray_valid.any():
            ...
            sigma_feature, cam_offset = self.renderDenModule(features=sigma_feature, temporal_mask=None, temporal_indices=None, iteration=iteration)
    ...
    def forward(self, rays_chunk, std_train, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1, rgb_train=None, cam_id =None, camlist=None,
                temporal_indices=None, static_branch_only=False, remove_foreground=False, iteration=None, test_cam_offset=0, **kwargs):
    ...
    def forward_seperatly(self, rays_chunk, std_train, cam_id, camlist, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1,
                          rgb_train=None, composite_by_points=False, temporal_indices=None, diff_calc=False, iteration=None,
        ...
        if ray_valid.any():
            ...
            elif self.time_head == 'timemlprender':
                sigma_feature, cam_offset = self.renderDenModule(features=sigma_feature, cam_id=camid_denmask, temporal_mask=None, temporal_indices=masked_temporal_indices, iteration=iteration)
            elif self.time_head == 'directdyrender':
        ...
        if app_spatio_mask.any():
            ...
            if self.time_head == 'timemlprender':
                valid_rgbs, cam_offset = self.renderModule(features=app_features, viewdirs=viewdirs[app_spatio_mask], cam_id=camid_rgbmask,
                                            ...
                                            temporal_mask=None, temporal_indices=masked_temporal_indices, test_cam_offset=test_cam_offset)
            elif self.time_head == 'directdyrender':
        ...
        ret_values = {'dynamic_mask': ray_mask,
                      'rgb_map': rgb_map,
                      'fraction': fraction,
                      'temporal_fraction': temporal_fraction,
                      'ray_wise_temporal_mask': ray_wise_temporal_mask,
                    'cam_offset': cam_offset if app_spatio_mask.any() and self.time_head == 'timemlprender' else torch.zeros([self.offset_size]),
                      }
        if nodepth == False:
            ret_values.update({
                'depth_map': depth_map,
    ...
    def forward_bullet(self, rays_chunk, std_train, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1, rgb_train=None, cam_id =None, camlist=None,
                temporal_indices=None, static_branch_only=False, remove_foreground=False, iteration=None, factor=2, start_sec=0, **kwargs):
        rays_chunk = rays_chunk.float()
        if std_train is not None:
            std_train = std_train.float()
        ...
        if static_branch_only:
            ...
            return retva
        return self.forward_seperatly_bullet(rays_chunk, std_train, cam_id, camlist, white_bg, is_train, ndc_ray, N_samples, rgb_train, temporal_indices=temporal_indices, iteration=iteration, factor=factor, start_sec=start_sec, **kwargs)

    def forward_seperatly_bullet(self, rays_chunk, std_train, cam_id, camlist, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1,
                          rgb_train=None, composite_by_points=False, temporal_indices=None, diff_calc=False, iteration=None,
        ...
        t_inputs = (np.arange(num_frames)) / factor + start_sec * 30
        ...
        if ray_valid.any():
            ...
            if self.time_head == 'forrier':
                ...
                sigma_feature, cam_offset = self.renderDenModule(features=sigma_feature, cam_id=camid_denmask, temporal_mask=None, temporal_indices=masked_temporal_indices, iteration=iteration, t_inputs=t_inputs)
            elif self.time_head == 'directdyrender':
                sigma_feature = self.renderDenModule(features=sigma_feature, temporal_mask=None, temporal_indices=masked_temporal_indices)
            validsigma = self.feature2density(sigma_feature)
            sigma[ray_valid] = validsigma
        ...
        if app_spatio_mask.any():
            ...
            if self.time_head == 'timemlprender':
                valid_rgbs, cam_offset = self.renderModule(features=app_features, viewdirs=viewdirs[app_spatio_mask], cam_id=camid_rgbmask,
                                            spatio_temporal_sigma_mask=app_mask[app_spatio_mask], iteration=iteration,
                                            temporal_mask=None, temporal_indices=masked_temporal_indices, t_inputs=t_inputs)
            elif self.time_head == 'directdyrender':
                valid_rgbs = self.renderModule(features=app_features, viewdirs=viewdirs[app_spatio_mask],
                                           spatio_temporal_sigma_mask=app_mask[app_spatio_mask], 
                                           temporal_mask=None, temporal_indices=masked_temporal_indices)
