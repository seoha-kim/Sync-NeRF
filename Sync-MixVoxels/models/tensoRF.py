...
class TensorVMSplit(MixVoxels):
    ...
    def _init_voxels(self, paramlist, type='normal', mean=0.0, std=0.1, a=-0.1, b=0.1):
        if type == 'none':
            return
        if type == 'normal':
            init = partial(torch.nn.init.normal_, mean=mean, std=std)
        if type == 'uniform':
            init = partial(torch.nn.init.uniform_, a=a, b=b)
        for param in paramlist:
            init(param)

    def init_svd_volume(self, res, device):
        self.dynamic_plane, self.dynamic_line = self.init_one_svd([16, 4, 4], self.gridSize, 0.1, device)
        self.frequency_plane, self.frequency_line = self.init_one_svd([16, 4, 4], self.gridSize, 0.1, device)
        ...
        # self._init_voxels(self.static_density_plane, type=self.args.init_static_voxel, mean=self.args.init_static_mean,
        #                   std=self.args.init_static_std, a=self.args.init_static_a, b=self.args.init_static_b)
        # self._init_voxels(self.static_density_line, type=self.args.init_static_voxel, mean=self.args.init_static_mean,
        #                   std=self.args.init_static_std, a=self.args.init_static_a, b=self.args.init_static_b)
        ...
        # self._init_voxels(self.density_plane, type=self.args.init_dynamic_voxel, mean=self.args.init_dynamic_mean,
        #                   std=self.args.init_dynamic_std, a=self.args.init_dynamic_a, b=self.args.init_dynamic_b)
        # self._init_voxels(self.density_line, type=self.args.init_dynamic_voxel, mean=self.args.init_dynamic_mean,
        #                   std=self.args.init_dynamic_std, a=self.args.init_dynamic_a, b=self.args.init_dynamic_b)
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            plane_coef.append(torch.nn.Parameter(
                scale * torch.randn((1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0]), dtype=(torch.float32 if self.amp else torch.float32) )))  #
            line_coef.append(torch.nn.Parameter(
                scale * torch.randn((1, n_component[i], gridSize[vec_id], 1), dtype=(torch.float32 if self.amp else torch.float32))))

        return torch.nn.ParameterList(plane_coef).to(device), torch.nn.ParameterList(line_coef).to(device)

    def get_optparam_groups(self, lr_dynamic_init=0.02, lr_dynamic_basis=0.001, cur_time_head=None, only_offset=False):
        if only_offset:
            return [{'params': self.cam_offset, 'lr': lr_dynamic_basis * self.args.camoffset_lr_factor}]
        ...
        grad_vars += [{'params': self.renderDenModule.mlp.parameters(), 'lr': lr_dynamic_basis}]
        grad_vars += [{'params':self.renderModule.mlp.parameters(), 'lr':lr_dynamic_basis}]
        if cur_time_head == 'timemlprender':
            grad_vars += [{'params': self.renderDenModule.time_mlp.time_mlp.parameters(), 'lr': lr_dynamic_basis * self.args.timemlp_lr_factor}]
            grad_vars += [{'params':self.renderModule.time_mlp.time_mlp.parameters(), 'lr':lr_dynamic_basis * self.args.timemlp_lr_factor}]
        if (cur_time_head == 'timemlprender') and self.args.cam_offset:
            print('optimize camera offsets')
            grad_vars += [{'params': self.cam_offset, 'lr': lr_dynamic_basis * self.args.camoffset_lr_factor}]
        return grad_vars
    ...
    def get_frequency_optparam_groups(self, lr = 0.02):
        grad_vars = []
        # parameters for dynamic prediction
        grad_vars += [{'params': self.frequency_line, 'lr': lr}]
        grad_vars += [{'params': self.frequency_plane, 'lr': lr}]
        return grad_vars
    ...
    def TV_loss_frequency(self, reg):
        return self.TV_loss_abstract(reg, self.frequency_plane)
    ...
    def compute_frequencies(self, xyz_sampled):
        return self.compute_single_value(xyz_sampled, self.frequency_plane, self.frequency_line)
    ...
    def upsample_volume_grid(self, res_target):
        ...
        self.frequency_plane, self.frequency_line = self.up_sampling_VM(self.frequency_plane, self.frequency_line, res_target)
    ...
    def shrink(self, new_aabb):
        ...
        for i in range(len(self.vecMode)):
            ...
            self.frequency_line[i] = torch.nn.Parameter(
                self.frequency_line[i].data[...,t_l[mode0]:b_r[mode0],:]
            )
            ...
            self.frequency_plane[i] = torch.nn.Parameter(
                self.frequency_plane[i].data[...,t_l[mode1]:b_r[mode1],t_l[mode0]:b_r[mode0]]
            )
