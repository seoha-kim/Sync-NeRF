...
def render_test(args):
    ...
    test_dataset = dataset(args.datadir, split='test', downsample=args.downsample_train, is_stack=True, #hold_id=[12,],
                           n_frames=args.n_frames, render_views=args.render_views, scene_box=args.scene_box,
                           temporal_variance_threshold=args.temporal_variance_threshold, white_bg = args.chromakey,
                           frame_start=args.frame_start, near=args.near, far=args.far, diffuse_kernel=args.diffuse_kernel)
    camlist = test_dataset.camlist
    test_camid = torch.Tensor([0.]) #test_dataset.all_camid
    ...
    if args.ckpt:
        if not os.path.exists(args.ckpt): 
            print('the ckpt does not exists!!')
            return  
        ckpt_path = args.ckpt
        ckpt = torch.load(args.ckpt, map_location=device)
    ...
    if args.ckpt:
        if args.no_load_timequery:
            tensorf.load_without_mlp(ckpt)
        else:
            tensorf.load(ckpt)
    ...
    if args.dense_alpha:
        ...
        convert_sdf_samples_to_ply(alpha.cpu()[...,150], f'{ckpt_path[:-3]}.ply', bbox=tensorf.aabb.cpu(), level=0.005)
        alpha = alpha.cpu().numpy()
        np.save(os.path.join(logfolder, 'dense_alpha.npy'), alpha)

    if args.render_train:
        os.makedirs(f'{logfolder}/imgs_train_all', exist_ok=True)
        train_dataset = dataset(args.datadir, split='train', downsample=args.downsample_train, is_stack=True,white_bg = args.chromakey, 
        ...
        with autocast(enabled=bool(args.amp)):
            PSNRs_test, PSNRs_STA_test, all_metrics = evaluation(train_dataset,tensorf, args, renderer, f'{logfolder}/imgs_train_all/', cam_id=test_camid,
                                    N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device, iteration=args.offset_start_iters+1, # temp
    ...
    if args.render_test:
        ...
        with autocast(enabled=bool(args.amp)):
            PSNRs_test, PSNRs_STA_test, all_metrics = evaluation(test_dataset,tensorf, args, renderer, f'{logfolder}/{args.expname}/imgs_test_all/', cam_id=test_camid,
                                    N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device, simplify=(args.n_frames>0), iteration=0, # temp
            ...
            with autocast(enabled=bool(args.amp)):
                if args.sigma_static_thresh < 1.0 or args.static_branch_only_initial:
                    evaluation_path(test_dataset, tensorf, args, c2ws, renderer, f'{logfolder}/{args.expname}/imgs_path_all/',
                                    N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device, cam_id=test_camid, iteration=0, # temp
                    ...
                    evaluation_path_efficient(test_dataset, tensorf, args, c2ws, renderer, f'{logfolder}/{args.expname}/imgs_path_all/',
                                        N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device)

def train_dynamics(args, tensorf, allrays, allrgbs, allstds, ndc_ray, nSamples, scaler, device, iter_ratio=1, dy_optimizer=None):
...
def eval_dynamics(args, tensorf, test_dataset, ndc_ray, nSamples, device):
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays), file=sys.stdout):
        ...
        camid_test = test_dataset.all_camid[idx].reshape(-1).to(device).contiguous()
...
def reconstruction(args):
    ...
    # args.expname = os.path.basename(args.config.split('.')[0]).split('/')[-1]
    # date = args.config.split('/')[-1].split('_')[0]
    ...
    if args.ckpt: 
        ...
        tensorf = eval(args.model_name)(args, aabb, reso_cur, device,
                    ...
                    static_point_detach=args.static_point_detach,
                    dynamic_pool_kernel_size=args.dynamic_pool_kernel_size,
                    time_head=args.time_head_pre,
                    filter_thresh=args.filter_threshold,
                    static_featureC=args.static_featureC,
                    camlist=camlist,
                    )
        n_iters = args.n_iters_pre
        cur_time_head = args.time_head_pre

    grad_vars = tensorf.get_optparam_groups(args.lr_dynamic_init, args.lr_dynamic_basis, cur_time_head=cur_time_head)
    static_grad_vars = tensorf.get_static_optparam_groups(args.lr_init, args.lr_basis)
    if args.lr_decay_iters > 0:
        lr_factor = args.lr_decay_target_ratio**(1/args.lr_decay_iters)
    else:
        args.lr_decay_iters = n_iters
    ...
    allrays, allrgbs, allstds, allcamids = train_dataset.all_rays, train_dataset.all_rgbs, train_dataset.all_stds, train_dataset.all_camid
    dynamicrays, dynamicrgbs, dynamicstds, dynamicmask = train_dataset.dynamic_rays, train_dataset.dynamic_rgbs, train_dataset.dynamic_stds, train_dataset.dynamic_mask
    if not args.ndc_ray:
    ...
    dy_optimizer = train_dynamics(args, tensorf, allrays, allrgbs, allstds, ndc_ray, nSamples, scaler, device)
    ...
    if args.temporal_sampler == 'simple':
        ...
        temporal_sampler = TemporalWeightedSampler(args.n_frames, args.n_train_frames, args.temperature_start,
                                                   args.temperature_end, n_iters, args.temporal_sampler_replace,
    ...
    cam_offset_log = open(f'{logfolder}/cam_offset.txt', 'w')
    for iteration in pbar:
        _time = time.time()
        if args.use_cosine_lr_scheduler:
            lr_factor = math.cos((iteration + 1.0) / n_iters * math.pi / 2) / math.cos((iteration + 0.0) / n_iters * math.pi / 2)
        gamma_current = iteration/n_iters * (args.gamma_end - args.gamma_start) + args.gamma_start
        ray_idx = trainingSampler.nextids(gamma=gamma_current)
        rays_train, rgb_train, std_train, cam_train = allrays[ray_idx].to(device).float(), allrgbs[ray_idx].to(device).float(), allstds[ray_idx].to(device).float(), allcamids[ray_idx].to(device).float()
        ...
        with autocast(enabled=bool(args.amp)):
            ...
            retva = renderer(rays_train, tensorf, chunk=current_batch_size, cam_id=cam_train,
                                    N_samples=nSamples, white_bg = args.chromakey, ndc_ray=ndc_ray,
        ...
        cam_offset_log.write(f'{iteration}, {retva.cam_offset.detach().cpu().numpy().tolist()} \n')
    ...
    if args.render_test:
        ...
        with autocast(enabled=bool(args.amp)):
            PSNRs_test, PSNRs_STA_test, all_metrics = evaluation(test_dataset,tensorf, args, renderer, f'{logfolder}/imgs_test_all/',
                                    N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device,  cam_id=test_camid,
                                    simplify=(args.n_frames>0), static_branch_only=args.static_branch_only, iteration=iteration)
    ...
    if args.render_path:
        ...
        with autocast(enabled=bool(args.amp)):
            if args.sigma_static_thresh < 1.0 or args.static_branch_only:
                ...
                evaluation_path(test_dataset, tensorf, args, c2ws, renderer, f'{logfolder}/imgs_path_all/', cam_id=test_camid,
                                N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device, iteration=iteration,
                                static_branch_only=args.static_branch_only, temporal_sampler=temporal_sampler)
            else:
                evaluation_path_efficient(test_dataset, tensorf, args, c2ws, renderer, f'{logfolder}/imgs_path_all/', iteration=iteration,
                                cam_id=test_camid, N_vis=-1, N_samples=-1, white_bg = args.chromakey, ndc_ray=ndc_ray,device=device)

    return tensorf


def test_offset_optimization(args, tensorf):
    # init dataset
    dataset = dataset_dict[args.dataset_name]
    time_dataset_start = time.time()
    train_dataset = dataset(args.datadir, split='train', downsample=args.downsample_train, is_stack=False, white_bg = args.chromakey,
                            n_frames=args.n_frames, scene_box=args.scene_box, temporal_variance_threshold=args.temporal_variance_threshold,
                            frame_start=args.frame_start, near=args.near, far=args.far, diffuse_kernel=args.diffuse_kernel, optimize_test=0)
    camlist = train_dataset.camlist
    time_dataset_end = time.time()
    print(f'Loading Train Dataset: {time_dataset_end-time_dataset_start}s')
    time_dataset_start = time_dataset_end
    ...
    n_iters = args.n_iters_test


    grad_vars = tensorf.get_optparam_groups(args.lr_dynamic_init, args.lr_dynamic_basis, only_offset=True)
    static_grad_vars = tensorf.get_static_optparam_groups(.0, .0)
    if args.lr_decay_iters > 0:
        lr_factor = args.lr_decay_target_ratio**(1/args.lr_decay_iters)
    else:
        args.lr_decay_iters = n_iters
    ...
    # tensorf.calc_init_alpha(tuple(reso_cur))
    cam_offset_log = open(f'{logfolder}/cam_offset_test.txt', 'w')
    for iteration in pbar:
        _time = time.time()
        if args.use_cosine_lr_scheduler:
            lr_factor = math.cos((iteration + 1.0) / n_iters * math.pi / 2) / math.cos((iteration + 0.0) / n_iters * math.pi / 2)
        ...
        if n_iters > 1000 and (iteration % 1000 == 0 or iteration == n_iters-1):
            tensorf.save(f'{logfolder}/{args.expname}_final.th', iteration, retva.cam_offset, optimizer, static_optimizer, dy_optimizer)
...
if __name__ == '__main__':
    ...
    if args.render_only and (args.render_test or args.render_path or args.dense_alpha):
        render_test(args)
    elif args.render_only and (args.render_train):
        render_test(args)
    else:
        logfolder = '{}/{}'.format(args.basedir, args.expname)
